...
const SCENES = [
  ...
  { key: 'night', label: '晚安祝福', icon: Moon, iconBg: 'bg-indigo-50' },
  ...
  { key: 'apology', label: '道歉和好', icon: MessageCircleHeart, iconBg: 'bg-emerald-50' },
      ...
      <ScrollView scrollY className="px-4 pt-4 pb-20">
        ...
        <View className="flex flex-row flex-wrap gap-2 mb-6">
          {TONES.map((tone) => (
            <Badge
              key={tone.key}
              className={selectedTone === tone.key ? 'bg-black text-white' : 'bg-gray-100 text-gray-700'}
