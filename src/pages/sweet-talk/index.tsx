...
const phaseColor: Record<string, string> = {
  '暧昧初期': 'bg-stone-100 text-stone-600',
...
const SweetTalkPage: FC = () => {
          ...
          {generating && !result && (
            <Card className="mb-4">
              <CardContent className="p-8 flex flex-col items-center justify-center">
                ...
                <Text className="block text-sm text-stone-500 mt-3">正在生成情话...</Text>
                <Text className="block text-xs text-stone-400 mt-1">AI 正在为你定制专属情话</Text>
              </CardContent>
            </Card>
          )}
          ...
          {result && (
            <>
              <View className="flex flex-row items-center justify-between mb-4">
                <Text className="block text-sm text-stone-500">{sceneLabel} · {TONES.find(t => t.key === selectedTone)?.label}</Text>
              ...
              {result.lines.map((line, idx) => (
                <Card key={idx} className="mb-3">
                  <CardContent className="p-4">
                    <Text className="block text-base text-stone-900 leading-relaxed mb-3">{line.content}</Text>
                    <View className="flex flex-row items-center justify-between">
                      <View className="flex flex-row items-center gap-2">
                        <Badge variant="secondary">
                          <Text className="text-xs">{line.style}</Text>
                        </Badge>
                        <Badge className={phaseColor[line.suitablePhase] || 'bg-stone-100 text-stone-600'}>
              ...
              <Card className="mb-4">
                <CardContent className="p-4">
                  <View className="flex flex-row items-center mb-3">
                    ...
                    <Text className="block text-base font-semibold text-stone-900 ml-2">发送小技巧</Text>
                  </View>
                  {result.tips.map((tip, idx) => (
                    <View key={idx} className="flex flex-row mb-2 last:mb-0">
                      <Text className="block text-sm text-stone-500 mr-2">{idx + 1}.</Text>
                      <Text className="block text-sm text-stone-600 flex-1">{tip}</Text>
                    </View>
                  ))}
                </CardContent>
              </Card>
  ...
  return (
    <View className="min-h-screen" style={{ backgroundColor: '#FFF9F0' }}>
      ...
      <ScrollView scrollY className="px-4 pt-4 pb-20">
        {/* 选择场景 */}
        <Text className="block text-sm font-semibold text-stone-900 mb-3">选择场景</Text>
        <View className="flex flex-row flex-wrap gap-3 mb-6">
          {SCENES.map((scene) => {
            ...
            return (
              <View
                key={scene.key}
                className={`rounded-2xl p-4 border-2 ${selectedScene === scene.key ? 'border-black bg-white' : 'border-stone-200 bg-white'}`}
                style={{ width: '46%' }}
                onClick={() => setSelectedScene(scene.key)}
              >
                <View className={`w-10 h-10 ${scene.iconBg} rounded-xl flex items-center justify-center mb-2`}>
                  <SceneIcon size={20} color="#44403C" />
                </View>
                <Text className="block text-sm font-semibold text-stone-900">{scene.label}</Text>
              </View>
            )
          })}
        </View>
        ...
        {/* 选择语气 */}
        <Text className="block text-sm font-semibold text-stone-900 mb-3">语气风格</Text>
        <View className="flex flex-row flex-wrap gap-2 mb-6">
          {TONES.map((tone) => (
            <Badge
              key={tone.key}
              className={selectedTone === tone.key ? 'bg-green-500 text-white' : 'bg-stone-100 text-stone-700'}
        ...
        {/* 自定义上下文 */}
        <Text className="block text-sm font-semibold text-stone-900 mb-3">补充要求（选填）</Text>
