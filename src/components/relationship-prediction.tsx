...
const RelationshipPrediction: FC<RelationshipPredictionProps> = ({ prediction }) => {
  const getTrendConfig = (trend: string) => {
    switch (trend) {
      ...
      default:
        return {
          icon: <Minus size={24} color="#78716C" />,
          label: '稳定',
    ...
    <View className={`rounded-xl border ${config.borderColor} ${config.bgColor} p-4`}>
      <View className="flex items-center justify-between mb-4">
        <View className="flex items-center gap-2">
          ...
          <Text className="block text-sm font-semibold text-stone-800">关系预测</Text>
        </View>
        <View className="flex items-center gap-2">
          {config.icon}
      ...
      <View className="flex items-center gap-2 mb-4">
        <View className="flex-1 h-2 bg-stone-200 rounded-full overflow-hidden">
          <View 
            className="h-full bg-stone-500 rounded-full"
            style={{ width: `${prediction.confidence}%` }}
          />
        </View>
        <Text className="block text-xs text-stone-500">置信度 {prediction.confidence}%</Text>
      </View>

      {/* 洞察 */}
      {prediction.insights.length > 0 && (
        <View className="mb-3">
          <Text className="block text-xs text-stone-500 mb-2">关键洞察</Text>
          {prediction.insights.map((insight, i) => (
            <View key={i} className="flex items-start gap-2 mb-1">
              <Text className="block text-xs text-stone-400">•</Text>
              <Text className="block text-sm text-stone-700">{insight}</Text>
            </View>
          ))}
        </View>
      )}
      ...
      {prediction.recommendations.length > 0 && (
        <View className="pt-3 border-t border-stone-200">
          <Text className="block text-xs text-stone-500 mb-2">建议</Text>
          {prediction.recommendations.map((rec, i) => (
            <View key={i} className="flex items-start gap-2 mb-1">
              <Text className="block text-xs text-stone-400">→</Text>
              <Text className="block text-sm text-stone-700">{rec}</Text>
            </View>
          ))}
        </View>
      )}
