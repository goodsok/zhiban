...
关键内容：${inputContent}`
  ...
  private async analyzeWithLLM(
    ...
    const systemPrompt = `你是一位高能量社交策略顾问，擅长生成【吸引型】话术，而非【追求型】话术。

核心原则：
1. 吸引 > 追求：话术要展示价值，创造好奇，而不是求关注
2. 平等位置感：不讨好、不迎合、不卑微，像朋友一样平等对话
3. 需求感管理：给对方空间，而不是施加回复压力
4. 框架清晰：每个话术有明确的目的和结构
...
根据用户MBTI调整话术风格：
- I型（INFJ/INFP/INTJ等）：走心深度型，话少精准有内涵，不热络但有力量，像一杯陈年好酒
- E型（ENTP/ENFP/ENTJ等）：活泼机智型，有感染力但不是讨好，像一杯气泡水
...
输出格式（JSON）：
{
  ...
  "advice": {
    "likeTiming": "点赞时机建议",
    "highEnergyComments": [
      {"style": "价值展示型", "text": "话术内容", "purpose": "这个话术的作用"},
      {"style": "好奇悬念型", "text": "话术内容", "purpose": "这个话术的作用"},
      {"style": "平等调侃型", "text": "话术内容", "purpose": "这个话术的作用"}
    ],
    ...
    try {
      ...
      return {
        ...
        advice: {
          ...
          highEnergyComments: [
            { style: '价值展示型', text: '这个角度拍的很有感觉，我上次也去了一个类似的地方', purpose: '展示自己的生活品味' }
          ],
