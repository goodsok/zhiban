...
6. interactionStyle 必须从以下选项中选择一个最匹配的：
   ...
   humor(幽默/开玩笑)、teasing(挑逗/逗弄)、flirting(调情/暧昧)、caring(关心/体贴)、
   vulnerable(展示脆弱/坦白)、challenging(激将/挑战)、pressuring(施压/逼问/索取)、
   cold(冷淡/疏远/无视)、empathetic(共情/理解/站在对方角度)、probing(试探/打听)、
   compliment(赞美/欣赏/夸奖)、apologetic(道歉/认错)、nostalgic(回忆/怀念过去)、
   jealous(吃醋/占有欲)、assertive(强势/主导/命令)、neutral(日常/中性/无明确信号)
...
{"safetyDelta":0,"desireDelta":0,"closenessDelta":0,"emotion":"neutral","emotionIntensity":50,"interactionStyle":"neutral"}`
  ...
  private detectTopicSensitivity(userMessage: string, dimensionValues: DimensionValue[]): string | null {
    const sensitivePatterns: Array<{ keywords: string[]; topic: string; condition: () => boolean }> = [
      {
        keywords: ['结婚', '嫁', '娶', '定下来', '未来规划'],
        topic: '婚姻/承诺话题',
        condition: () => {
          const openness = this.getDimensionValue(dimensionValues, 'personality_openness') || 50
          return openness < 40 // 低开放性对婚姻话题敏感
        },
      },
      {
        keywords: ['前任', '前男友', '前女友', '初恋'],
        topic: '前任/过去感情',
        condition: () => true, // 所有人对前任话题都敏感
      },
      {
        keywords: ['你怎么总是', '你每次都', '你从来'],
        topic: '被指责/批评',
        condition: () => {
          const agreeableness = this.getDimensionValue(dimensionValues, 'personality_agreeableness') || 50
          return agreeableness > 60 // 高宜人性更怕被批评
        },
      },
      {
        keywords: ['工资', '赚多少', '存款', '房子'],
        topic: '金钱/物质话题',
        condition: () => {
          const openness = this.getDimensionValue(dimensionValues, 'personality_openness') || 50
          return openness < 45
