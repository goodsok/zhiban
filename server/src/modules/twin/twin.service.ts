...
{"safetyDelta":0,"desireDelta":0,"closenessDelta":0,"emotion":"neutral","emotionIntensity":50}`
  ...
  private generateHint(
    ...
    // --- 1. 安全感变化解读 ---
    if (safetyDelta <= -8) {
      insights.push(`安全感骤降${Math.abs(safetyDelta)}点，你的话让她感到明显压力`)
    } else if (safetyDelta <= -4) {
      insights.push(`安全感下降了${Math.abs(safetyDelta)}点，她有点不安`)
    } else if (safetyDelta >= 4) {
      insights.push(`安全感上升${safetyDelta}点，她在你身边开始放松了`)
    } else if (safetyDelta >= 1) {
      insights.push('安全感微升，她对你的信任在缓慢积累')
    } else if (safetyDelta < 0) {
      insights.push(`安全感略降${Math.abs(safetyDelta)}点，她有些不自在`)
    }

    // --- 2. 渴望度变化解读（依恋类型差异化） ---
    if (desireDelta >= 6) {
      if (attachment === 'anxious') {
        insights.push(`渴望度急升${desireDelta}点——焦虑型依恋，你的话让她更想抓住你`)
      } else {
        insights.push(`渴望度大幅上升${desireDelta}点，她突然很想靠近你`)
      }
    } else if (desireDelta <= -6) {
      if (attachment === 'avoidant' || attachment === 'fearful') {
        insights.push(`渴望度降了${Math.abs(desireDelta)}点——${attachmentLabel[attachment]}面对压力会本能退缩`)
      } else {
        insights.push(`渴望度降了${Math.abs(desireDelta)}点，她对你的兴趣在减退`)
      }
    } else if (desireDelta >= 3) {
      insights.push('渴望度在升高，她心里有点想靠近你了')
    } else if (desireDelta <= -3) {
      insights.push('渴望度在降低，她开始想要保持距离')
    }

    // --- 3. 亲密度变化 + 门控反弹 ---
    if (closenessDelta > 0 && newRel.closeness === Math.floor(newRel.safety * getAttachmentParams(dimensionValues).gateFactor)) {
      insights.push('亲密度触达安全感的上限，想再近一步需要先提升她的安全感')
    } else if (closenessDelta >= 2) {
      insights.push('你们的距离在缩小，她愿意让你靠近了一些')
    } else if (closenessDelta <= -2) {
      insights.push('她主动拉开了距离')
    }
    ...
    if (prevRel.stage !== newRel.stage) {
      ...
      insights.push(`关系阶段变化：${stageLabels[prevRel.stage]} → ${stageLabels[newRel.stage]}`)
    }

    // --- 5. 张力状态解读 ---
    if (newRel.tension > 50) {
      insights.push('内心非常纠结——渴望和距离之间的差距巨大')
      suggestions.push('别急着推进，先帮她降低内心的矛盾感')
    } else if (newRel.tension > 30) {
      if (newRel.desire > newRel.closeness) {
        insights.push('她想靠近但不敢——渴望远超实际距离')
        if (attachment === 'avoidant' || attachment === 'fearful') {
          suggestions.push('给她空间，不要追问，等她自己靠近')
        } else if (attachment === 'anxious') {
          suggestions.push('给她确认感，一句"我在"比长篇大论更有效')
        }
      } else {
        insights.push('距离比渴望更近，她有点不自在')
      }
    }

    // --- 6. 态度锚点 + 情绪组合解读 ---
    if (newEmo.attitudeAnchor === 'longing') {
      insights.push('她内心渴望靠近但不敢，嘴上说的和心里想的不一样')
      if (attachment === 'avoidant') suggestions.push('别逼她承认，用行动靠近比语言更有效')
    } else if (newEmo.attitudeAnchor === 'guarded' && newRel.safety < 40) {
      insights.push('她在防御，安全感很低，不会轻易敞开')
      suggestions.push('少聊感受多聊事，让她在无压力对话中慢慢卸下防备')
    } else if (newEmo.attitudeAnchor === 'cold' && newRel.safety < 25) {
      insights.push('她对你非常冷淡，安全感几乎为零')
      suggestions.push('不要试图讨好或强行接近，保持适度存在感就好')
    } else if (newEmo.attitudeAnchor === 'fond' && newRel.safety > 50) {
      insights.push('她对你有好感，而且安全感够高，这是个好信号')
    }

    // --- 7. 回避型"墙"提示 ---
    if (attachment === 'avoidant' && newRel.safety >= 45 && newRel.safety <= 65) {
      suggestions.push('⚠️ 回避型墙区(45-65)：她可能会突然冷淡，不是你的问题，是她的防御机制')
    }

    // --- 8. 情绪剧烈变化 ---
    if (newEmo.emotionIntensity >= 80 && prevEmo.emotionIntensity < 50) {
      insights.push(`情绪强度突然飙到${newEmo.emotionIntensity}%，你触到了她的某个敏感点`)
    }

    // --- 组装 ---
    if (insights.length === 0) {
      // 没有显著变化时，给出一个默认温和提示
      insights.push('这轮对话没有引起明显波动，关系平稳')
      if (newRel.safety < 30) suggestions.push('安全感还很低，继续温和地建立信任')
    }
