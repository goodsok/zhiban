...
{"safetyDelta":0,"desireDelta":0,"closenessDelta":0,"emotion":"neutral","emotionIntensity":50,"interactionStyle":"neutral"}`
  ...
  private async checkAndGenerateProactiveMessage(
    ...
    // 连续对话中不生成主动消息：如果最近5条消息中有主动消息特征（assistant连续出现），则跳过
    const recentHistory = await this.getRecentHistory(matchId, 5)
    const recentAssistantCount = recentHistory.filter(m => m.role === 'assistant').length
    if (recentAssistantCount >= 3) {
      // 连续对话中，assistant 消息密度高，不生成主动消息
      return null
    }
...
只输出消息内容，1-2句话，不要引号不要解释。`
      ...
      if (proactiveMsg) {
        const supabase = getSupabaseClient()
        // 主动消息同时存入 chat_history，确保 LLM 上下文连贯
